...
local function ClassListToTable(name)
	local tbl={}
	local recipeClsList, recipeClsCnt = GetClassList(name);
...
function STATUSPOINTCHECK_QUESTCHECK(statuspoint)
	...
	local quests=ClassListToTable("QuestProgressCheck_Auto")
	questList={}
	for i,v in ipairs(quests) do
		
		local questDetail = GetClassByType('QuestProgressCheck_Auto', v.ClassID);
		if(questDetail.Success_StatByBonus and questDetail.Success_StatByBonus>0) then
			questList[#questList+1]=v.ClassID
		end
	...
	for k, questNo in pairs(questList) do
		...
		if STATUSPOINTCHECK_QUESTCLEARCHECK(questNo) then
			...
			if mapprop then
				...
				mapBody = "??[Level:"..tostring(questCls.Level).."]"
			end
		end
	...
	title:SetText("{#000000}{s16}"..titleBody.."("..getPoint.."/"..sumPoint.."pts){/}{/}")
end
...
function STATUSPOINTCHECK_STATUSQUESTCHECK(status)
	...
	questList={}
	local rewards=ClassListToTable("reward_property")
	for i,v in ipairs(rewards) do
		if v.Property == "STR" or 
		v.Property == "CON" or 
		v.Property == "DEX"  or 
		v.Property == "INT"  or 
		v.Property == "MNA" or 
		v.Property == "MHP" or 
		v.Property == "MSP"   then
			local quest=GetClass("QuestProgressCheck_Auto",v.ClassName)
			questList[#questList+1]=quest.ClassID
		end
	end
...
function STATUSPOINTCHECK_ZEMINACHECK(zemina)
	...
	local clsList=ClassListToTable("Map")

	for i, v in ipairs(clsList) do
		if(STATUSPOINTCHECK_HAS_ZEMINA(v.ClassName))then
			mapList[#mapList+1]=v.ClassName
		end
	end
	
	for k, mapClassName in ipairs(mapList) do
...
function STATUSPOINTCHECK_HAS_ZEMINA(mapClassName)
