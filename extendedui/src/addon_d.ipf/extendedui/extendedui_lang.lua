...
extui.language.defaultFile = [[{
	...
	"jpn": {
		...
		"data": {
			...
			"chooseSkin": "スキン",
			"showFrame": "フレームを表示",
			"vistrue": "チェックに応じて表示・非表示状態が保存されます",
			"visfalse": "チェックにかかわらず表示・非表示状態は保存されません",
			"loadMessage": "アドオン読み込みメッセージを非表示",
			"loadMessageDesc": "ロード完了時に表示される\"ExtendedUI Loaded\"のメッセージを削除します",
			"hideJoy": "Set1,Setボタンを非表示",
			"hideJoyDesc": "ジョイスティックの\"Set 1\"/\"Set 2\" ボタンを非表示にします",
			"showExp": "経験値を表示",
			"showExpDesc": "経験値バーにマウスカーソルを合わせると、実際の経験値を表示します。 (マップ変更時に更新されます)",
			"disablePop": "レシピポップアップを削除",
			"disablePopDesc": "レシピポップアップを削除",
			"lockQuest": "クエストログ位置を固定",
			"lockQuestDesc": "クエストが追加・削除された際にログ位置が動かなくなるようにします。",
			"buffs": "バフ",
			"bIconSize": "バフアイコンのサイズ",
			"extBuff": "バフアイコン表示を拡張",
			"extBuffDesc": "バフアイコンを最大３０個まで表示できるようにします",
			"buffAmt": "1行当たりのバフアイコン数",
			"buffAmtDesc": "バフアイコンが指定した数、表示されるごとに行が作成されます。 (「バフアイコン表示を拡張」がONの時のみ有効です)",
			"buffSec": "常に秒で表示",
			"buffSecDesc": "○○分の代わりに○○秒と表示します",
			"confirmReset": "設定をリセットしてもよろしいですか？{nl}全フレームがデフォルト位置に戻ります。",
			"options": "設定",
			"lang": "言語",
			"resetFrame": "リセット",
			"author": "翻訳者"
