    ...
    EBI_try_catch{
        try = function()
            ...
            if jobIcon ~= nil then
                ...
                touch:SetTextTooltip("To show AOP menu,Press LSHIFT + RBtn ");
            end
...
function AOP_ON_TIMER(frame)
    EBI_try_catch{
        try = function()
            ...
            if(ui.GetFrame("party_recommend"):IsVisible()==1 and g.casting==false)then
                AOP_CASTING(true)
            end
...
function AOP_RENDER_PARTY_MEMBER(frame, pic, partyMemberInfo,idx, inc, ox, oy)
    ...
    if myInfo and (myInfo:GetMapID() ~= partyMemberInfo:GetMapID() or myInfo:GetChannel() ~= partyMemberInfo:GetChannel()) then
        local text=AOP_GENERATE_TEXT(frame, "location" .. id,
...
function AOP_RBTNUP(parent, ctrl)
    if (keyboard.IsKeyPressed("LSHIFT") == 1) then
        ...
        ui.AddContextMenuItem(context, "Mode:Horizontal", "AOP_CHANGEMODE(0)");
        ui.AddContextMenuItem(context, "Mode:Vertical", "AOP_CHANGEMODE(1)");
        ui.AddContextMenuItem(context, "Mode:Counterclockwise", "AOP_CHANGEMODE(2)");
        ui.AddContextMenuItem(context, "Mode:Auto Horz", "AOP_CHANGEMODE(8)");
        ui.AddContextMenuItem(context, "Mode:Auto Vert", "AOP_CHANGEMODE(9)");
