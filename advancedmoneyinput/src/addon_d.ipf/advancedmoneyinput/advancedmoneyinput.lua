...
function ADVANCEDMONEYINPUT_INIT(addoncollab)
    EBI_try_catch {
        try = function()
            ...
            if addoncollab then
                base = 50*4+15
                size = 50
                DepositSkin:Resize(DepositSkin:GetWidth(), 80)
            else
                base = 240
                DepositSkin:Resize(DepositSkin:GetWidth(), 50)
            end
            local btnc =
                DepositSkin:CreateOrGetControl(
                "button",
                "btnc",
                editMoney:GetX() + base,
                editMoney:GetY() + editMoney:GetHeight() - 4,
                size,
                25
            )
            base=base-size
        ...
        try = function()
            EBI_try_catch {
                try = function()
                    ...
                    WarehouseManager = _G["ADDONS"][string.upper("Charbon")][string.upper("WarehouseManager")]
                end,
                catch = function(error)
                    WarehouseManager=nil
                end
            }
            if WarehouseManager then
                ReserveScript("ADVANCEDMONEYINPUT_INIT(true)",0.2)
            else
                ADVANCEDMONEYINPUT_INIT(false)
            end
        end,
        catch = function(error)
            ERROUT(error)
