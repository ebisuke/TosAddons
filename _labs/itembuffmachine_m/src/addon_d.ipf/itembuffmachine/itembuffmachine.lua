...
--On click new weap
function ITEMBUFFMACHINE_WEAPBUFF_ONBUTTON()
    EBI_try_catch {
        try = function()
            ...
            if price == 0 then
                ...
                ui.MsgBox('Do you want buff all weapons ?{nl}{#FFFF00}{ol}cost:' .. tostring(price), string.format('ITEMBUFFMACHINE_WEAPBUFF_DO_SELECT()'), 'None')
            end
        end,
        catch = function(error)
            ERROUT(error)
    ...
    EBI_try_catch {
        try = function()
            ...
            if price == 0 then
                ...
                ui.MsgBox('Do you want buff all items ?{nl}{#FFFF00}{ol}cost:' .. tostring(price), string.format('ITEMBUFFMACHINE_ITEMBUFF_DO_SELECT()'), 'None')
            end
        end,
        catch = function(error)
            ERROUT(error)
...
--new buff weap
function ITEMBUFFMACHINE_WEAPBUFF_CHECK_Squire_EquipmentTouchUp(item)
    if item.GroupName == 'Weapon' then
        return 1
    end
...
--New weapon buff **
function ITEMBUFFMACHINE_WEAPBUFF_DO_SELECT()
    g.items={}
    local equiplist = session.GetEquipItemList()
    for i = 0, equiplist:Count() - 1 do
        local equipItem = equiplist:GetEquipItemByIndex(i)
        ...
        if spotName ~= nil then
            ...
            if tempobj ~= nil then
                local obj = GetIES(tempobj)
                local fn=ITEMBUFFMACHINE_WEAPBUFF_CHECK_Squire_EquipmentTouchUp
...
---WEap buff 
function ITEMBUFFMACHINE_WEAPBUFF()
    EBI_try_catch {
        try = function()
            local equipItemIESID=g.items[g.itemcursor]
            local frame = ui.GetFrame('itembuffopen')
