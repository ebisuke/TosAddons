    ...
    EBI_try_catch{
        try = function()
            ...
            if(handle~=nil)then
                ...
                local kills=GetMonKillCount(GetMyPCObject(),type)
                st=st..monCls.Name..":"..tostring(kills).."{nl}"
            ...
            -- local list, cnt = SelectObject(GetMyPCObject(), 50, 'ENEMY')
            -- local i
            -- --frame:ShowWindow(0)
            -- for i = 1, cnt do
            --      --local targetinfo = info.GetTargetInfo( handle );
            ...
            --          --local dpcInfo = dummyPC.GetInfo(GetHandle(obj));
            --          print(tostring(targetinfo))
            --          --local skill= session.GetSkillByName(sklName)
                     
            --          local actor = world.GetActor( GetHandle(obj))
            --          local isAble = geMCC.IsAbleToUseSkill(actor, GetMyActor(), type);
            --          --local skill= GetSkil.GetSkillByGuid(sklCls:GetIESID());	
            --          local curTime =  skill:GetCurrentCoolDownTime();
            --          local totaltime = skill:GetTotalCoolDownTime();
