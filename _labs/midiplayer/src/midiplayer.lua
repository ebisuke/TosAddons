...
function MIDIPLAYER_ON_TIMER()
    EBI_try_catch{
        try = function()
            ...
            local iframe = ui.GetFrame("instrument_keyboard");
            ...
            if g.play then
                ...
                g.prevtime = os.clock();
                local allover = true
                g.elapsed = g.elapsed + g.score[1] * timediff * (1 / 60 * g.tempo)--60BPM base
                        ...
                        if g.soundcount < 1 then
                            ...
                            if event[1] == 'note_on' and event[5] ~= 0 then
                                ...
                                if (not g.ch and event[3] ~= 9) or isIn(event[3], g.ch) then
                                    --ならす
                                    local note = g.notebind_active[event[4] + g.pitch]
                                    
                                    if note ~= nil and g.prevnote[note] == nil then
                                        if g.onoff[note] ~= 0 then
                                            g.onoff[note] = 1
                                            g.prevnote[note] = note
											g.soundcount=g.soundcount+1
                                        end
