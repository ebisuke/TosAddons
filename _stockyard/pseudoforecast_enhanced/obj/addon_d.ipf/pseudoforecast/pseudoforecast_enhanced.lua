            ...
            if pc ~= nil then
                ...
                if enemyCount > 0 then
                    for i = 1, enemyCount do
                        if (enemyList[i].ClassName == "ENEMY" or enemyList[i].ClassName == "PAD") then
                    ...
                    if (#padList > 0) then
                        ...
                        for i = 1, #padList do
                            ...
                            if (data.pads[guid] == nil) then
                                ...
                                PSEUDOFORECAST_PADSKILL(pad, data.skill)
                                data.pads[guid] = pad
                            end
                        end
                    end
...
function PSEUDOFORECAST_PADSKILL(actor,pad, skillclsid)
    EBI_try_catch{
        try = function()
            ...
            if (xmlskls) then
                ...
                for i = 1, #xmlskls do
                    local xmlskl = xmlskls[i]
                    CHAT_SYSTEM("IN"..tostring(xmlskl.timestart))
                    PSEUDOFORECAST_PADSKILLACTION(actor,pad, class.ClassName, i)
                end
...
function PSEUDOFORECAST_SKILLACTION(actor,pad, classname, index)
...
function PSEUDOFORECAST_PADSKILLACTION(actor, classname, index)
    local xmlskl = PSEUDOFORECAST_DATA[classname][index]
    local duration = math.max(0.5, (xmlskl.timeend - xmlskl.timestart) / 1000.0)
    local push = xmlskl.length
    if (xmlskl.timestart % 10 == 9) then
    ...
    if (xmlskl.scptype == "MONSKL_CRE_PAD") then
